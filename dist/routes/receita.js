import { Router } from 'express';
import * as ReceitaController from '@/controllers/ReceitaController.js';
import { autenticarChef } from '@/middlewares/auth.js';
import { HttpError } from '@/errors/index.js';
const router = Router();
// List all receitas
router.get('/', async (req, res, next) => {
    try {
        const receitas = await ReceitaController.listarReceitas();
        res.json(receitas);
    }
    catch (error) {
        next(new HttpError('Unable to read receitas', 400));
    }
});
// Get receita by id
router.get('/:id', async (req, res, next) => {
    try {
        const id = Number(req.params.id);
        const receita = await ReceitaController.obterReceita(id);
        res.json(receita);
    }
    catch (error) {
        next(new HttpError(error.message, 404));
    }
});
// Get receitas from logged chef
router.get('/chef/minhas', autenticarChef, async (req, res, next) => {
    try {
        const receitas = await ReceitaController.obterReceitasChef(req.chef.id);
        res.json(receitas);
    }
    catch (error) {
        next(new HttpError(error.message, 400));
    }
});
// Create receita
router.post('/', autenticarChef, async (req, res, next) => {
    try {
        const createdReceita = await ReceitaController.criarReceita({ ...req.body, chef_id: req.chef.id });
        res.status(201).json(createdReceita);
    }
    catch (error) {
        next(new HttpError(error.message, 400));
    }
});
// Update receita
router.put('/:id', autenticarChef, async (req, res, next) => {
    try {
        const id = Number(req.params.id);
        const updatedReceita = await ReceitaController.atualizarReceita({ ...req.body, id, chef_id: req.chef.id });
        res.json(updatedReceita);
    }
    catch (error) {
        next(new HttpError(error.message, 400));
    }
});
// Delete receita
router.delete('/:id', autenticarChef, async (req, res, next) => {
    try {
        const id = Number(req.params.id);
        await ReceitaController.deletarReceita(id, req.chef.id);
        res.sendStatus(204);
    }
    catch (error) {
        next(new HttpError(error.message, 400));
    }
});
export default router;
//# sourceMappingURL=receita.js.map