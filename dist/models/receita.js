...
async function vincularEtiquetas(db, receitaId, etiquetas) {
    ...
    for (const nome of lista) {
        ...
        if (!etiquetaObj)
            throw new Error(`Etiqueta "${nome}" não encontrada`);
        await db.run(`INSERT INTO receita_etiqueta (receita_id, etiqueta_id) VALUES (?, ?)`, [receitaId, etiquetaObj.id]);
    }
}
...
async function update(data) {
    ...
    if (id && img && tagsInput && title && time && servings && ingredients && steps) {
        ...
        if (!atual)
            throw new Error('Receita não encontrada');
        if (chef_id && atual.chef_id !== chef_id)
...
async function remove(id, chef_id) {
    ...
    if (!atual)
        throw new Error('Receita não encontrada');
    if (chef_id && atual.chef_id !== chef_id)
        throw new Error('Sem permissão para deletar esta receita');
